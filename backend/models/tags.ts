...
class Tags {
  ...
  async renameTag(
    ...
    const rows = ((result.rows ?? result) as any[]).map((row) => ({
      id: row.id as string,
      // -> `siteId` and `tags` are both what the page rules match on: upstream's rules gained TAG and
      //    TAGALL kinds, so a rule can now deny exactly the pages a rename would touch
      siteId,
      path: row.path as string,
      locale: row.locale as string,
      tags: (row.tags ?? []) as string[]
